...
PATH = "/Users/nicksorenson/School/prog_languages/assign2/"

LispAtoms = ["True", "False", "nil"]
LispFunctions = ['+','-','*','/','if','not','and','or','eq','<','atom','cons','first','rest','quote','eval']
...
def evalLispBlock(block, oneBinding):
    ...
    if len(block) == 0:  # empty block, return None
        return
    code = block[0]
    if isinstance(code, list) and code[0] == 'defun':  # put mapping from name to arguments, body
        oneBinding[code[1]+"_defun"] = (code[2], code[3])
        if Trace:
            print("Create function->definition binding: %s = [%s, %s]" % (
...
def evalLispHelp(code, bindings, depth):
    ...
    # numerical functions
    if fun == '+':
        return evalLisp(code[1], bindings, depth + 1) + evalLisp(code[2], bindings, depth + 1)
    if fun == '-':
        return evalLisp(code[1], bindings, depth + 1) - evalLisp(code[2], bindings, depth + 1)
    if fun == '*':
        return evalLisp(code[1], bindings, depth + 1) * evalLisp(code[2], bindings, depth + 1)
    ...
    if fun == 'and':
        return evalLisp(code[1], bindings, depth + 1) and evalLisp(code[2], bindings, depth + 1)
    if fun == 'or':
        return evalLisp(code[1], bindings, depth + 1) or evalLisp(code[2], bindings, depth + 1)
    if fun == 'eq':  # only works between atoms
        return simpleEqual(evalLisp(code[1], bindings, depth + 1), evalLisp(code[2], bindings, depth + 1))
    if fun == '<':
        return evalLisp(code[1], bindings, depth + 1) < evalLisp(code[2], bindings, depth + 1)
    if fun == 'atom':
...
def findValue(name, bindings, depth):
    ...
    for oneBinding in bindings:
        if name in oneBinding:  # found its value
            if Trace:
                print("|  " * depth + "Found Value of %s as %s" % (name, oneBinding[name]))
